...
tbs-map = {
  ...
  ? "id_certificate_hash" : [1* $hashed-uri-map], ; optional hash of certificate of other ID assertion signers
}
----
...
to mitigate the threat of the authorized Claim Signature being stripped and replaced.  The hash is calcuated over the leaf certificate expressed in the 
same representation as used in the signature structure (e.g., DER for X.509).  

`id_certificate_hash` (optional) is an array of leaf-certificates-hashes of other ID assertion signing signing keys in this manifest.  
ID-assertion-generators can set this field to mitigate the threat of later ID assertions generated later being stripped and replaced.  Note that this field uses the 
