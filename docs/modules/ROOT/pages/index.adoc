...
*Version 1.0 Draft 25 March 2024* · xref:_version_history[]
...
* `tbs` (“to be signed”) contains a set of data to be signed by this _<<_credential_holder,credential holder>>_. This structure is a CBOR key-value map that contains:
** `referenced_assertions` _(required)_ contains a list of other assertions selected by the credential subject. This is referred to as `tbs.referenced_assertions` throughout this specification.
...
The content of `tbs` is a data structure which is presented to the _<<_credential_holder,credential holder>>_ for signature. The list of _<<_referenced_assertions,referenced assertions>>_ contained within `tbs` MUST include a _<<hard_binding,hard binding>>_ assertion as described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hard_bindings++[Section 9.2, “Hard bindings,”] of the C2PA technical specification.

IMPORTANT: A single signature from the _<<_credential_holder,credential holder>>_ covers the entire `tbs` data structure, including the _list_ of assertions. An implementation MUST NOT present each assertion separately for signature.

For each assertion listed in the `tbs.referenced_assertions` field of the *<<_identity_assertion,identity assertion>>,* an assertion with the same `url`, `alg`, and `hash` values MUST also be listed in the `assertions` field of the _<<C2PA claim>>_ in which the *<<_identity_assertion,identity assertion>>* appears.

The list of _<<_referenced_assertions,referenced assertions>>_ in the `tbs.referenced_assertions` field of the *<<_identity_assertion,identity assertion>>* is necessarily a subset of the assertions listed in the `assertions` field of the _<<C2PA claim>>_ because the *<<_identity_assertion,identity assertion>>* MUST NOT refer to itself. The list of assertions MAY be further subsetted. For example, different _<<_credential_holder,credential holders>>_ could attest to different assertions in a single _<<C2PA Manifest>>._
...
----
identity = {
  "tbs": $tbs-map, ; content to be signed by credential holder
...
tbs-map = {
   "sig_type": tstr .size (1..max-tstr-length),  ; a string identifying the signature type
  "referenced_assertions": [1* $hashed-uri-map], ; the assertions vouched for in the signature
...
IMPORTANT: Future minor version updates (1.1, 1.2, etc.) to this specification MAY add new fields to the `tbs-map` description, provided that such new data members are optional and there is a well-specified default meaning that is compatible with the 1.0 version of this specification. Such updates to the specification SHOULD continue to use the `cawg.identity` assertion label.

The `sig_type` field MUST contain a value described in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].

The `hashed-uri-map` for `referenced_assertions` is defined in link:++https://c2pa.org/specifications/specifications/1.3/specs/C2PA_Specification.html#_hashed_uris++[Section 8.3.1, “Hashed URIs,”] of the C2PA technical specification.
...
Assertions can be referenced in two ways: if `hashed-uri-map.hash` is present, then the hash value should be set to the assertion-hash in the normal way.  
If `hashed-uri-map.hash` is not set, then verifiers will 
...
to mitigate the threat of the authorized Claim Signature being stripped and replaced.  The hash is calcuated over the leaf certificate of the Claim Signing 
key using the same representation as used in the signature structure (i.e.,, DER for X.509).  

`id_certificate_hash` (optional) serves two purposes.  An ID-assertion creator can use this field to prevent unauthorized ID assertions being added 
to the Claim after the ID assertion has been signed.  To do this, `id_certificate_hash` should be set to an array of `hashed-uri-map` structure referencing 
all of the authorized assertions, but the `hashed-cert-uri-map.hash` fields of all entries should be absent.

This field can also be used to stop unauthorized replacement or creation of ID assertions that are created and signed after the  

is an array of leaf-certificates-hashes of other ID assertion signing signing keys in this manifest.  
ID-assertion-generators can set this field to mitigate the threat of later ID assertions generated later being stripped and replaced.  This field can also 
be used to stop additional assertions being added to the claim after the ID signature has been finalized.
If this field is used, the array must have an entry for every assertion referenced in the associated claim.  In contrast to other 

Note that this field uses the 
`hashed-uri-map` data structure, but in this case, the URI-hash is the hash of the Claim Signing key's leaf certificate. this field is used, all assertions must be referenced by JUMBF-URI, but not all 

TODO: Need to guard against new assertions being added.  Perhaps referenced_assertions could be either a hash value or NULL.  If NULL, the assertion must be present but thehash is 
not checked.
...
ID assertion generator _can_ include the hash of the Claim Signature signing key certificate.  If this is present, validators will
...
=== Presenting the `tbs` data structure for signature

Prior to presenting the _<<_credential_holder,credential holder>>_ with the `tbs` data structure for signature, the _<<_referenced_assertions,referenced assertions>>_ MUST themselves be created. This process is described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_creating_assertions#_creating_assertions++[Section 10.3.1, “Creating assertions,”] of the C2PA technical specification.
...
The `tbs` data structure MUST be presented to be signed by the _<<_credential_holder,credential holder>>_ corresponding to each *<<_identity_assertion,identity assertion>>.* This process is described in more detail in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].
...
The `tbs` data structure MUST be unchanged from the data structure that was presented to the _<<_credential_holder,credential holder>>_ for signature.
...
An *<<_identity_assertion,identity assertion>>* MUST contain valid CBOR that satisfies the `identity` rule in xref:_cbor_schema[xrefstyle=full]. The `cawg.identity.cbor.invalid` error code SHALL be used to report assertions that do not follow this rule.

For each entry in `tbs.referenced_assertions`, the validator MUST verify that the same entry exists in either the `created_assertions` or `gathered_assertions` entry of the _<<C2PA claim>>._ (For version 1 claims, the entry must appear in the `assertions` entry.) The `cawg.identity.assertion.mismatch` error code SHALL be used to report violations of this rule.

The validator SHOULD verify that no entry in `tbs.referenced_assertions` is duplicated. The `cawg.identity.assertion.duplicate` error code SHALL be used to report violations of this rule.

The validator MUST ensure that `tbs.referenced_assertions` contains at least one _<<_hard_binding,hard binding>>_ assertion as described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hard_bindings++[Section 9.2, “Hard bindings”] of the C2PA technical specification. The `cawg.identity.hard_binding_missing` error code SHALL be used to report a missing _<<_hard_binding,hard binding>>_ assertion.
...
* Any reference to the _claim_ MUST be replaced with the CBOR serialization of the `tbs` field from the *<<_identity_assertion,identity assertion>>.* The `tbs` data structure MUST be serialized as described in link:++https://www.rfc-editor.org/rfc/rfc8949.html#name-core-deterministic-encoding++[Section 4.2.1, “Core Deterministic Encoding”,] of RFC 8949: Concise Binary Object Representation. The resulting byte string is presented to the _<<_credential_holder,credential holder>>_ for signature.
...
* Any reference to the _claim_ MUST be replaced with the CBOR serialization of the `tbs` field from the *<<_identity_assertion,identity assertion>>.*
* Any reference to the _claim generator_ MUST be replaced with the _<<_actor,actor>>_ whose _X.509 certificate_ is being used for this assertion.
